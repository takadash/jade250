...
kabukiData.order("createData", true)
  ...
  .then(function(results) {
    ...
    for (var i = 0; i < results.length; i++, cnt7++) {
      ...
      marker7[i] = new google.maps.Marker({
    ...
    function markerEvent1(i) {
      marker7[i].addListener('click', function() { // マーカーをクリックしたとき
        if (currentInfoWindow) { //currentInfoWindowに値があるならば
          currentInfoWindow.close(); //開いていた吹き出しを閉じる
        }
        infoWindow7[i].open(map, marker7[i]); // 吹き出しの表示
        currentInfoWindow = infoWindow7[i];
      });
    }
...
function stamp_push1(i) {
  ...
  stamp_lat = stamplat7[i];
  stamp_lng = stamplng7[i];
